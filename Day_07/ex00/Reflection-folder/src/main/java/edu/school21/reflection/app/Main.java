...
public class Main {
	...
	private static final List<String> packages = Arrays.asList("edu.school21.reflection.classes");
	private static final Set<Class> allClasses = new HashSet<>();

	public static void main(String[] args) {
		try {
			packages.forEach(pack -> allClasses.addAll(Utils.findAllClasses(pack)));
			System.out.println("Classes:");
			allClasses.forEach(cl -> System.out.println(Utils.getClassName(cl)));
			chooseClass();
			outputMethodFields();
			creatingObject();
			updatingFields();
	...
	private static void chooseClass() {
		while (true) {
			...
			if (list.isEmpty()) {
				...
				currClass = list.get(0);
				break;
			}
		}
	}

	private static void outputMethodFields() {
		System.out.println("-----------------");
		fields = Arrays.asList(currClass.getDeclaredFields());
		methods = Arrays.stream(currClass.getMethods())
				.filter(m -> m.toString().contains(Utils.getPackageName(currClass)))
	...
	private static void creatingObject() throws InvocationTargetException,
			InstantiationException, IllegalAccessException {
		List<Object> parameters = new ArrayList<>();
		System.out.println("-----------------");
		System.out.println("Let's create an object.");
		Constructor constructor = Arrays.stream(currClass.getConstructors()).
	...
	private static void updatingFields() throws IllegalAccessException {
		while (true) {
			System.out.println("-----------------");
	...
	private static void choiceMethod() throws InvocationTargetException, IllegalAccessException {
		while (true) {
			List<Object> parameters = new ArrayList<>();
			System.out.println("-----------------");
